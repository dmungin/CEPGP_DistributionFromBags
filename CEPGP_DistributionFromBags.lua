...
function CEPGP_DFB_AnnounceWinner(isWinner)
	if isWinner then
		...
		if player == PLAYER_NAME then
			...
			-- Fix for trading with players on same realm as you --
			local indexOfPlayerRealm = string.find(player, "-".. realmName, 1, true)
			local tradePlayer = player
			if indexOfPlayerRealm ~= nil then
				tradePlayer = string.sub(player, 1, indexOfPlayerRealm - 1)
			end
			if CheckInteractDistance(tradePlayer, 2) then
				CEPGP_DFB_confirmation:Hide();
				InitiateTrade(tradePlayer)
			else
